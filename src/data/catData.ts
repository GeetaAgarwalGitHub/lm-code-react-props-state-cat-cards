...
import { CatImageProps } from "../components/cat_image";
...
const CatImages: CatImageProps[] = [
  {
    img: cat1,
    alt_Text: "Describe this cat!",
...
catData.forEach((cat, index) => {
  console.log(CatImages.length + "kkkkkkkkkkkkkk");
  const id = uuidv4();
  cat.id = id;
  //  CatImages[index].alt_Text = id;
});

export default { catData, CatImages };
